...
```
> The local copy of the `/ubuntu/fsiam` directory will be referred to as `FSIAM_DOCKERFILE_HOME` from this point onwards.
...
- Execute `docker build` command as shown below.
    ```
    docker build --build-arg BASE_PRODUCT_VERSION=<IS BASE PRODUCT VERSION> --build-arg WSO2_OB_ACCELERATOR_DIST_URL=<URL_OF_THE_HOSTED_LOCATION/FILENAME> --build-arg OB_TRUSTED_CERTS_URL=<URL_OF_THE_HOSTED_LOCATION/FILENAME> --build-arg WSO2_OB_KEYSTORES_URL=<URL_OF_THE_HOSTED_LOCATION/FOLDER_NAME> -t wso2-fsiam:4.0.0 .
    ```
...
- If you want to Docker mount the database connector file to the container run below command.
  ```
  docker run -it -v <LOCAL_PATH_TO_CONNECTOR:<PATH_TO_IS_LIBS_FOLDER_IN_CONTAINER>  -p 9446:9446 wso2-fsiam:4.0.0
  ```
